  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const submitProfile = async () => {
        ...
        try {
          message.value = 'Submitting profile...';
          ...
          if (response.data.profile && response.data.profile.id) {
            emit('submit-success', response.data.profile.id);
          }
        } catch (error) {
          console.error('Error:', error);
          if (error.response?.data) {
            errors.value = error.response.data.errors || [error.response.data.message || 'Profile update failed'];
          } else {
            errors.value = ['An error occurred while updating your profile. Please try again later.'];
          }
        }
      };
